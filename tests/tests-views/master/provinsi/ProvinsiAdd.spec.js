...
import ProvinsiAdd from '@/views/master/provinsi/ProvinsiAdd.vue';
...
describe('ProvinsiAdd.vue', () => {
    ...
    it('should handle success save', async () => {
        const wrapper = factory();
        ProvinsiService.post.mockResolvedValue({
    ...
    it('should get detail provinsi', async () => {
        const wrapper = factory();
        ProvinsiService.getById.mockResolvedValue({
            data: { code: 200, message: 'Success' }
        });

        wrapper.vm.dataInput.id = 1;
        await wrapper.vm.getDetailProvinsi();

        await flushPromises();

        expect(ProvinsiService.getById).toHaveBeenCalled();
    });

    it('should handle success edit', async () => {
        const wrapper = factory();
        ProvinsiService.put.mockResolvedValue({
            data: { code: 200, message: 'Success' }
        });

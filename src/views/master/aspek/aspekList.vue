...
import AspekService from './service/aspek.js';
...
const actionMenuItems = ref([
    {
        ...
        icon: 'pi pi-pencil',
        command: () => {
            onClickEditAspek(selectedData.value);
        }
    },
    {
        separator: true
    },
    {
        ...
        class: 'text-red-500',
        command: () => {
            onClickDeleteAspek(selectedData.value);
        }
    }
]);
...
function getData() {
    ...
    AspekService.get(apiParams)
        .then((response) => {
            if (response.data.code === 200) {
                allData.value = response.data.data.map(item => ({
                    id: item.id,
                    'Kode Aspek': item.kodeAspek,
                    'Nama Aspek': item.namaAspek,
...
function applyFiltersAndPagination() {
    ...
    if (search.value && search.value.trim()) {
        ...
        filteredData = filteredData.filter(item => 
            item['Kode Aspek'].toLowerCase().includes(searchTerm) ||
            item['Nama Aspek'].toLowerCase().includes(searchTerm) ||
            item['Deskripsi'].toLowerCase().includes(searchTerm)
        );
    }
    
    // Apply column filters
    if (columnFilters.value.kodeAspek) {
        filteredData = filteredData.filter(item => 
            item['Kode Aspek'].toLowerCase().includes(columnFilters.value.kodeAspek.toLowerCase())
        );
    }
    if (columnFilters.value.namaAspek) {
        filteredData = filteredData.filter(item => 
            item['Nama Aspek'].toLowerCase().includes(columnFilters.value.namaAspek.toLowerCase())
        );
    }
...
const onClickEditAspek = (data) => {
    formData.value = {
        id: data.id,
        kodeAspek: data['Kode Aspek'],
        namaAspek: data['Nama Aspek'],
...
const onSubmitForm = (data) => {
    ...
    if (!formData.value.kodeAspek || formData.value.kodeAspek.trim() === '') {
        errors.push('Kode Aspek tidak boleh kosong');
    }
    
    if (!formData.value.namaAspek || formData.value.namaAspek.trim() === '') {
        errors.push('Nama Aspek tidak boleh kosong');
    }
    ...
    const payload = {
        kodeAspek: formData.value.kodeAspek,
        namaAspek: formData.value.namaAspek,
        deskripsi: formData.value.deskripsi
    };

    const serviceCall = isEditMode.value 
        ? AspekService.put(formData.value.id, payload)
        ...
        .then((response) => {
            if (response.data.code === 200) {
                toast.add({
                    severity: 'success',
                    summary: 'Berhasil',
                    detail: isEditMode.value ? 'Data aspek berhasil diupdate' : 'Data aspek berhasil ditambahkan',
                    life: 3000
                });
                clearForm();
...
function onClickDeleteAspek(data) {
    dataAspek.value = data;
...
function deleteData(accept) {
    ...
    AspekService.delete(dataAspek.value.id)
        .then((response) => {
            if (response.data.code === 200) {
                accept();
                toast.add({
                    severity: 'success',
                    summary: 'Berhasil',
                    detail: 'Data aspek berhasil dihapus',
                    life: 3000
                });
                getData(); // Refresh all data from server
            } else {
                ...
                toast.add({
                    severity: 'error',
                    summary: 'Gagal',
                    detail: response.data.message || 'Gagal menghapus data aspek',
                    life: 3000
                });
            }
        })
...
<template>
    <div>
        <div class="font-semibold text-xl mb-4">Aspek</div>
            ...
            >
                <div class="grid grid-cols-12 gap-2 mb-2">
                    ...
                    <div class="col-span-3">
                        <InputText
                            autocomplete="off"
                            v-model="formData.kodeAspek"
                            placeholder="Masukkan Kode Aspek"
                            :disabled="isEditMode"
                            :class="[
                                'w-full',
                                { 'p-invalid': showValidationErrors && (!formData.kodeAspek || formData.kodeAspek.trim() === '') }
                            ]"
                            @input="showValidationErrors = false"
                        />
                        <Message
                            v-if="showValidationErrors && (!formData.kodeAspek || formData.kodeAspek.trim() === '')"
                            severity="error"
                            size="small"
                            variant="simple"
                        >
                            Kode Aspek tidak boleh kosong
                        </Message>
                    </div>
                </div>
                <div class="grid grid-cols-12 gap-2 mb-2">
                    ...
                    <div class="col-span-3">
                        <InputText
                            autocomplete="off"
                            v-model="formData.namaAspek"
                            placeholder="Masukkan Nama Aspek"
                            :class="[
                                'w-full',
                                { 'p-invalid': showValidationErrors && (!formData.namaAspek || formData.namaAspek.trim() === '') }
                            ]"
                            @input="showValidationErrors = false"
                        />
                        <Message
                            v-if="showValidationErrors && (!formData.namaAspek || formData.namaAspek.trim() === '')"
                            severity="error"
                            size="small"
                            variant="simple"
                        >
        ...
        <div class="card">
            ...
            >
                ...
                >
                    <template #header>
                        <div class="w-full">
                            <div class="font-semibold text-gray-900 mb-2">Kode Aspek</div>
                            <div v-if="showFilters" class="w-full">
                                <InputText 
                                    v-model="columnFilters.kodeAspek"
                                    placeholder="Filter kode aspek..."
                                    class="w-full"
                ...
                >
                    <template #header>
                        <div class="w-full">
                            <div class="font-semibold text-gray-900 mb-2">Nama Aspek</div>
                            <div v-if="showFilters" class="w-full">
                                <InputText 
                                    v-model="columnFilters.namaAspek"
                                    placeholder="Filter nama aspek..."
                                    class="w-full"
                                    @input="onFilterChange"
                                    size="small"
                                />

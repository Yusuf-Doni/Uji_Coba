...
import JenisRapatService from './service/JenisRapatService.js';
        ...
        .then((response) => {
            if (response.data.code === 200) {
                allData.value = response.data.data.map(item => ({
                    id: item.id,
                    'Kode Rapat': item.kodeJenisRapat,
                    'Nama Rapat': item.jenisRapat,
...
function applyFiltersAndPagination() {
    ...
    if (search.value && search.value.trim()) {
        ...
        filteredData = filteredData.filter(item => 
            item['Kode Rapat'].toLowerCase().includes(searchTerm) ||
            item['Nama Rapat'].toLowerCase().includes(searchTerm) ||
            item['Deskripsi'].toLowerCase().includes(searchTerm)
        );
    }
    
    // Apply column filters
    if (columnFilters.value.kodeJenisRapat) {
        filteredData = filteredData.filter(item => 
            item['Kode Rapat'].toLowerCase().includes(columnFilters.value.kodeJenisRapat.toLowerCase())
        );
    }
    if (columnFilters.value.jenisRapat) {
        filteredData = filteredData.filter(item => 
            item['Nama Rapat'].toLowerCase().includes(columnFilters.value.jenisRapat.toLowerCase())
        );
    }
...
const onClickEditRapatKerja = (data) => {
    formData.value = {
        id: data.id,
        kodeJenisRapat: data['Kode Rapat'],
        jenisRapat: data['Nama Rapat'],
...
const onSubmitForm = (data) => {
    ...
    if (!formData.value.kodeJenisRapat || formData.value.kodeJenisRapat.trim() === '') {
        errors.push('Kode Rapat tidak boleh kosong');
    }
    
    if (!formData.value.jenisRapat || formData.value.jenisRapat.trim() === '') {
        errors.push('Nama Rapat tidak boleh kosong');
    }
    ...
    const payload = {
        kodeJenisRapat: formData.value.kodeJenisRapat,
        jenisRapat: formData.value.jenisRapat,
...
<template>
    <div>
        <div class="font-semibold text-xl mb-4">Rapat Kerja</div>
            ...
            >
                <div class="grid grid-cols-12 gap-2 mb-2">
                    ...
                    <div class="col-span-3">
                        <InputText
                            autocomplete="off"
                            v-model="formData.kodeJenisRapat"
                            placeholder="Masukkan Kode Rapat"
                            :disabled="isEditMode"
                            :class="[
                                'w-full',
                                { 'p-invalid': showValidationErrors && (!formData.kodeJenisRapat || formData.kodeJenisRapat.trim() === '') }
                            ]"
                            @input="showValidationErrors = false"
                        />
                        <Message
                            v-if="showValidationErrors && (!formData.kodeJenisRapat || formData.kodeJenisRapat.trim() === '')"
                            severity="error"
                            size="small"
                            variant="simple"
                        >
                            Kode Rapat tidak boleh kosong
                        </Message>
                    </div>
                </div>
                <div class="grid grid-cols-12 gap-2 mb-2">
                    ...
                    <div class="col-span-3">
                        <InputText
                            ...
                            :class="[
                                ...
                                { 'p-invalid': showValidationErrors && (!formData.jenisRapat || formData.jenisRapat.trim() === '') }
                            ]"
                            @input="showValidationErrors = false"
                        />
                        <Message
                            v-if="showValidationErrors && (!formData.jenisRapat || formData.jenisRapat.trim() === '')"
                            severity="error"
                            size="small"
                            variant="simple"
                        >
        ...
        <div class="card">
            ...
            >
                ...
                >
                    <template #header>
                        <div class="w-full">
                            <div class="font-semibold text-gray-900 mb-2">Kode Rapat</div>
                            <div v-if="showFilters" class="w-full">
                                <InputText 
                                    v-model="columnFilters.kodeJenisRapat"
                                    placeholder="Filter kode jenis rapat..."
                                    class="w-full"
                ...
                >
                    <template #header>
                        <div class="w-full">
                            <div class="font-semibold text-gray-900 mb-2">Nama Rapat</div>
                            <div v-if="showFilters" class="w-full">
                                <InputText 
                                    v-model="columnFilters.jenisRapat"
                                    placeholder="Filter nama rapat kerja..."
                                    class="w-full"
                                    @input="onFilterChange"
                                    size="small"
                                />

...
const selectedJenisArahan = ref(null);
const selectedYear = ref(null); // Remove default year filter
const dateRange = ref(null);

// Status options - mapped to integer values as per API specification
const statusOptions = ref([
    { label: 'In Progress', value: 1 },
    { label: 'Selesai Berkelanjutan', value: 2 },
    { label: 'Selesai', value: 3 }
]);

// Jenis Arahan options - will be loaded from API
const jenisArahanOptions = ref([]);
...
watch([searchText, selectedStatus, selectedJenisArahan, selectedYear, dateRange], () => {
    applyFilters();
}, { deep: true });
...
// ==========================================
const loadJenisArahanOptions = async () => {
    try {
        const response = await reportService.getRUPSArahanTypes();
        
        if (response.data && response.data.success === true && response.data.data) {
            // Map API response to dropdown options
            jenisArahanOptions.value = response.data.data.map(item => ({
                label: item.description, // "Dewan Komisaris" or "Pemegang Saham"
                value: item.code // 1 or 2
            }));
        } else {
            console.error('No data in jenis arahan response:', response);
            throw new Error('Data jenis arahan tidak ditemukan');
        }
    } catch (error) {
        console.error('Error loading jenis arahan options:', error);
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Gagal memuat data jenis arahan: ' + error.message,
            life: 3000
        });
        
        // Fallback to default options if API fails
        jenisArahanOptions.value = [
...
const applyFilters = async () => {
    const filters = {
        ...
        arahanRups: selectedJenisArahan.value,
...
const loadReportData = async (filters = {}) => {
    try {
        ...
        // Build filter parameters according to RUPS API specification
        const apiFilters = reportService.buildRUPSFilterParams(filters);
        
        const response = await reportService.getRUPSReport(apiFilters);
        ...
        console.error('Error loading RUPS report data:', error);
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Gagal memuat data report RUPS: ' + error.message,
            life: 3000
        });
    } finally {
...
const getJenisArahanSeverity = (jenisArahan) => {
    // Map API response values to severity colors
    switch (jenisArahan) {
        case 'Pemegang Saham': return 'info';
        case 'Dewan Komisaris': return 'warning';
        // Legacy support for old values
        case 'PS': return 'info';
        case 'Dekom': return 'warning';
        default: return 'secondary';
    }
};

const getJenisArahanLabel = (jenisArahan) => {
    // Map API response values to display labels
    switch (jenisArahan) {
        case 'Pemegang Saham': return 'PS (Pemegang Saham)';
        case 'Dewan Komisaris': return 'Dekom (Dewan Komisaris)';
        // Legacy support for old values
        case 'PS': return 'PS (Pemegang Saham)';
        case 'Dekom': return 'Dekom (Dewan Komisaris)';
        default: return jenisArahan;
    }
};
...
const exportToExcel = async () => {
    try {
        ...
        // Get current filters with correct parameter names for RUPS API
        const filters = {
            search: searchText.value,
            statusTindakLanjut: selectedStatus.value,
            arahanRups: selectedJenisArahan.value,
        ...
        const response = await reportService.exportRUPSToExcel(apiFilters);
        ...
        const filename = `Report_RUPS_${new Date().toISOString().slice(0, 10)}.xlsx`;
        await reportService.generateExcelFromData(exportData, filename, 'rups');
...
const clearFilters = async () => {
    ...
    selectedJenisArahan.value = null;
...
onMounted(async () => {
    ...
    // Load jenis arahan options from API
    await loadJenisArahanOptions();
    ...
    <div>
        ...
        <div class="mb-4">
            <Button
                ...
                class="mb-2"
            />
            <span v-if="!showFilters && (searchText || selectedStatus || selectedJenisArahan || selectedYear || dateRange)" class="ml-3 text-sm text-blue-600">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <Dropdown
                                v-model="selectedJenisArahan"
                                :options="jenisArahanOptions"
                                optionLabel="label"
                                optionValue="value"
            ...
            <template #content>
                ...
                <div v-if="isLoading" class="flex justify-center items-center h-64">
                    <div class="text-center">
                        ...
                        <p class="text-gray-600">Memuat data report RUPS...</p>
                    </div>
                </div>
                ...
                <div v-else>
                    ...
                    >
                        ...
                        <Column field="jenisArahan" header="Jenis Arahan (PS / Dekom)" :sortable="true" style="width: 180px">
                            <template #body="slotProps">
                                <div class="text-sm">
                                    {{ getJenisArahanLabel(slotProps.data.arahanTypeDesc) }}
                                </div>
                            </template>
                        </Column>
                ...
                <div v-if="!isLoading" class="mt-4 p-3 bg-gray-50 rounded-lg">
                    <div class="flex justify-between items-center text-sm text-gray-600">
                        ...
                        <span v-if="searchText || selectedStatus || selectedJenisArahan || selectedYear || dateRange">

...
function applyFiltersAndPagination() {
    ...
    if (tahunParam.value) {
        filteredData = filteredData.filter(item => {
            if (item.tanggalRapatFrom || item.tanggalRapat) {
                const itemYear = new Date(item.tanggalRapatFrom || item.tanggalRapat).getFullYear();
    ...
    if (columnFilters.value.tanggalRapat) {
        ...
        filteredData = filteredData.filter(item => {
            const itemDate = new Date(item.tanggalRapatFrom || item.tanggalRapat).toISOString().split('T')[0];
    ...
    if (sortField.value && sortOrder.value) {
        filteredData.sort((a, b) => {
            ...
            if (sortField.value === 'tanggalRapat') {
                aValue = new Date(a.tanggalRapatFrom || a.tanggalRapat);
                bValue = new Date(b.tanggalRapatFrom || b.tanggalRapat);
            }
...
function onClickEditJadwal(data) {
    router.push({
        path: `/transaction/rapat-rups-rkap/edit/${data.id}`,
        query: {
...
function onClickDetailJadwal(data) {
    router.push({
        path: `/transaction/rapat-rups-rkap/detail/${data.id}`,
        query: {
            Id: data.id,
            Agenda: data.agenda,

...
async function getData() {
    ...
    try {
        ...
        if (response.data && response.data.success) {
            ...
            // Temporary: Direct assignment for testing
...
function applyFiltersAndPagination() {
    ...
    if (tahunParam.value) {
        filteredData = filteredData.filter(item => {
            ...
            const dateField = item.tanggalRapatFrom || item.tanggalRapat;
            if (dateField) {
    ...
    if (jenisRapatParam.value) {
        ...
        if (jenisRapatLabel) {
            filteredData = filteredData.filter(item => {
                ...
                const matchesByLabel = item.jenisRapat === jenisRapatLabel;
                const matchesById = item.jenisRapatId === parseInt(jenisRapatParam.value);
                const matchesByStringId = item.jenisRapatId === jenisRapatParam.value;
                
                const matches = matchesByLabel || matchesById || matchesByStringId;
    ...
    if (columnFilters.value.jenisRapat) {
        filteredData = filteredData.filter(item => 
            item.jenisRapat === columnFilters.value.jenisRapat
        );
    }
    ...
    if (columnFilters.value.tanggalRapat) {
        ...
        filteredData = filteredData.filter(item => {
            // Handle both old and new date field formats
            const itemDate = item.tanggalRapatFrom ? 
                new Date(item.tanggalRapatFrom).toISOString().split('T')[0] : 
                new Date(item.tanggalRapat).toISOString().split('T')[0];
            return itemDate === filterDate;
        });
    }
    
    if (columnFilters.value.tahun) {
        filteredData = filteredData.filter(item => 
            item.tahun === columnFilters.value.tahun
        );
    }
    ...
    if (columnFilters.value.jenisRapatId) {
        filteredData = filteredData.filter(item => 
            item.jenisRapatId === columnFilters.value.jenisRapatId
        );
    }
    ...
    if (sortField.value && sortOrder.value) {
        filteredData.sort((a, b) => {
            ...
            // Handle date fields - support both old and new formats
            if (sortField.value === 'tanggalRapat') {
                // Use tanggalRapatFrom if available, otherwise fallback to tanggalRapat
                const aDateField = a[sortField.value + 'From'] || a[sortField.value];
                const bDateField = b[sortField.value + 'From'] || b[sortField.value];
                aValue = new Date(aDateField);
                bValue = new Date(bDateField);
            }
    ...
    // Fallback: Jika tidak ada data setelah filtering tapi ada data asli, tampilkan semua data
    if (filteredData.length === 0 && allData.value.length > 0) {
        datas.value = allData.value.slice(startIndex, endIndex);
...
function onClickEditJadwal(data) {
    router.push({
        ...
        query: {
            ...
            tanggalRapatFrom: data.tanggalRapatFrom ? new Date(data.tanggalRapatFrom) : null,
            tanggalRapatTo: data.tanggalRapatTo ? new Date(data.tanggalRapatTo) : null,
            tahun: data.tahun,
...
function onClickDetailJadwal(data) {
    ...
    router.push({
        path: `/transaction/jadwal-rapat-korporat/detail/${jenisRapatParam.value}/${data.id}`,
        query: {
            Id: data.id,
            Agenda: data.agenda,
            JenisRapat: data.jenisRapat,
...
function onClickUpload(data) {
    router.push({
        ...
        query: {
            id: data.id,
            agenda: data.agenda,
            tanggalRapatFrom: data.tanggalRapatFrom,
            tanggalRapatTo: data.tanggalRapatTo,
            statusDecision: data.statusDecision,
            jenisRapatId: data.jenisRapatId,
            jenisRapat: data.jenisRapat,
            linkMeeting: data.linkMeeting,
...
const getActionMenuItems = () => {
    ...
    // Conditionally add Detail Jadwal if statusDecision is not 0
    if (selectedData.value?.statusDecision !== 0) {
        baseItems.push({
            label: 'Detail Jadwal',
    ...
    // {
    //     label: 'Approval Radir',
    //     icon: 'pi pi-check-circle',
    //     class: 'text-green-600',
    //     command: () => {
    //         onClickApprovalRadir(selectedData.value);
    //     }
    // },
    ...
    if (currentRole === '2' || currentRole === 2) {
        ...
        // Other roles: Show Upload
        return [
            {
                label: 'Upload',
                icon: 'pi pi-upload',
                command: () => {
                    onClickUpload(selectedData.value);
                }
            },
            ...baseItems
        ];
        ...
        <div class="card">
            ...
            >
                ...
                >
                    <template #header>
                        <div class="w-full">
                            <div class="font-semibold text-gray-900 mb-2">Jenis Rapat</div>
                            <div v-if="showFilters" class="w-full">
                                <Select 
                                    v-model="columnFilters.jenisRapat"
                ...
                >
                    <template #header>
                        <div class="w-full">
                            <div class="font-semibold text-gray-900 mb-2">Tahun</div>
                            <div v-if="showFilters" class="w-full">
                                <Select 
                                    v-model="columnFilters.tahun"
                ...
                >
                    ...
                    <template #body="{ data }">
                        <Tag 
                            :value="data.statusDecision === 1 ? 'Menggunakan Decision' : data.statusDecision === 2 ? 'Tidak Menggunakan Decision' : 'Belum Diputuskan'" 

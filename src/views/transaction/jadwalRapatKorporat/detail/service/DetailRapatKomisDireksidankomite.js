...
class DetailRapatKomisDireksidankomiteService {
    ...
    getByIdFlagApprove(id) {
        return http.get(`/${this.url}/getJadwalRapatDetail/${id}`);
    }
    ...
    getListPic() {
        ...
        // Hanya role 2 yang bisa mengakses service ini
        if (userRole != 2) {
            console.warn('Access denied: Only role 2 can access getListPic');
            return Promise.reject({
                message: 'Anda tidak memiliki akses untuk melihat daftar PIC',
                code: 'ACCESS_DENIED',
                status: 403
            });
        }
    ...
    getArahanRapatById(id) {
        return http.get(`${this.url}/arahanRapat/${id}`);
    }
